...
access(all) contract TestablePythTrading {
    ...
    // Trading Portfolio Resource using real Pyth-format prices
    access(all) resource TradingPortfolio: TradingPortfolioPublic {
        ...
        // Execute buy action using Pyth-format prices
        access(self) fun executeBuyAction(asset: String, amount: UFix64): String {
            // Get price using Pyth integration (real on mainnet/testnet, mock on emulator)
            let priceInfo = TestablePythTrading.getPythPrice(asset: asset)
            if priceInfo == nil {
            ...
            // Update holdings with Pyth price
        ...
        // Execute sell action using Pyth price
        ...
        access(all) fun withdrawFlow(amount: UFix64): @FlowToken.Vault {
            ...
            let withdrawn <- self.flowVault.withdraw(amount: amount) as! @FlowToken.Vault
            self.updateTotalValue()
            emit FlowWithdrawn(portfolio: self.portfolioId, amount: amount)
            return <- withdrawn
        }

        // Update portfolio value using Pyth prices
        access(self) fun updateTotalValue() {
            var totalAssetValue = 0.0

            for asset in self.holdings.keys {
                if let holding = self.holdings[asset] {
                    // Use Pyth price for valuation
                    if let priceInfo = TestablePythTrading.getPythPrice(asset: asset) {
                        let currentPrice = priceInfo.getFormattedPrice()
                        totalAssetValue = totalAssetValue + (holding.quantity * currentPrice)
                    }
                    // If no price available, asset value = 0 (realistic)
                }
            }
    ...
    // Get Pyth price (works on emulator with mock data, real data on mainnet/testnet)
    access(all) fun getPythPrice(asset: String): TestablePythIntegration.PriceInfo? {
        return TestablePythIntegration.getPrice(feedId: asset)
    }

    // Get Pyth price with max age check
    access(all) fun getPythPriceWithMaxAge(asset: String, maxAge: UInt64): TestablePythIntegration.PriceInfo? {
        return TestablePythIntegration.getPriceNoOlderThan(feedId: asset, maxAge: maxAge)
    }

    // Update Pyth prices (admin function for testing)
    access(all) fun updateMockPrice(feedId: String, price: Int64, expo: Int32, confidence: UInt64) {
        TestablePythIntegration.setMockPrice(feedId: feedId, price: price, expo: expo, confidence: confidence)
    }
    ...
    access(all) fun getSupportedAssets(): [String] {
        return TestablePythIntegration.getSupportedFeeds()
    }

    init() {
        self.totalPortfolios = 0
        self.PortfolioStoragePath = /storage/TestablePythTradingPortfolioV1
        self.PortfolioPublicPath = /public/TestablePythTradingPortfolioV1
    }
}