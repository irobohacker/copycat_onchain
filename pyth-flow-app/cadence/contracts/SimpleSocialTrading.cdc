...
access(all) contract SimpleSocialTrading {
    ...
    access(all) resource TradingPortfolio: TradingPortfolioPublic {
        ...
        init(owner: Address) {
            ...
            self.holdings = {}
            self.flowBalance = 100.0  // Start with some FLOW for testing
            self.totalValue = 100.0
            self.pendingActions = {}
        }
        ...
        access(self) fun executeBuyAction(asset: String, amount: UFix64): String {
            let priceData = SimpleSocialTrading.getAssetPrice(asset: asset)
            if priceData == nil {
        ...
        access(self) fun updateTotalValue() {
            ...
            for asset in self.holdings.keys {
                if let holding = self.holdings[asset] {
                    if let priceData = SimpleSocialTrading.getAssetPrice(asset: asset) {
    ...
    init() {
        ...
        // Set storage paths
        self.PortfolioStoragePath = /storage/SimpleSocialTradingPortfolioV1
        self.PortfolioPublicPath = /public/SimpleSocialTradingPortfolioV1
    }
}